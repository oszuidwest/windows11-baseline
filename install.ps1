...
#===============================================================
# Call all sub-scripts with admin rights
#===============================================================
...
if (Test-Path $scriptDirectory) {
    ...
    foreach ($scriptFile in $scriptFiles) {
        ...
        try {
            $arguments = "-File `"$($scriptFile.FullName)`" -purpose `"$purpose`" -ownership `"$ownership`" -password `"$deploy_user_password`" -computername `"$deploy_computer_name`" -workgroup `"$workgroup`""
